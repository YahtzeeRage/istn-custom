...
def test(args):
    ...
    with torch.no_grad():
        for index, batch_samples in enumerate(dataloader_test):
            ...
            #save those matricees
            affineMatricesWarped.append([(config.stn).getAffineMatrix()])
    ...
    if args.no_refine == False:
        ...
        for index, batch_samples in enumerate(dataloader_test):
            ...
            with torch.no_grad():
                ...
                # added this to get the matrices
                affineMatricesRefined.append([refine_config.stn.getAffineMatrix()])
...
if __name__ == '__main__':
    ...
    if args.test is not None:
        affineMatricesWarped = []
        affineMatricesRefined = []
        test(args)
        if (len(affineMatricesWarped) != 60 or len(affineMatricesRefined) != 60):
            print(len(affineMatricesWarped))
            # print(affineMatricesWarped)
            print(len(affineMatricesWarped))
            # print(affineMatricesRefined)
            raise Exception("uh oh didn't get my affineMatrices")
        pickle.dump(affineMatricesWarped, open(os.path.join(args.out, "affineMatrices_warped_" + args.loss + ".dat"), "wb"))
        pickle.dump(affineMatricesRefined, open(os.path.join(args.out, "affineMatrices_refined_" + args.loss + ".dat"), "wb"))
